...
@objc public enum AlertStyle: Int {
    case success, error, warning, customImage, none
}

@objc open class SweetAlert: UIViewController {
    ...
    init() {
        ...
        //Retaining itself strongly so can exist without strong reference
        strongSelf = self
    }
    ...
    @objc open override func viewWillLayoutSubviews() {
    ...
    func closeAlert(_ buttonIndex: Int) {
        ...
        UIView.animate(withDuration: 0.5, delay: 0.0, options: UIView.AnimationOptions.curveEaseOut, animations: { () -> Void in
            ...
            //Releasing strong reference of itself.
            self.strongSelf = nil
        }
    }
    ...
    @objc open func showAlert(_ title: String) -> SweetAlert {
        _ = showAlert(title, subTitle: nil, style: .none, customImage: "")
        return self
    }
